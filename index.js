...
function createSymbolForKey(key) {
   ...
   //Якщо символ з таким ключем вже існує, він повертає його, в іншому випадку створює новий символ з даним ключем та додає його в реєстр
...
let rangeObject = {
   ...
   to: 7,
   [Symbol.iterator]() {
      //початкове значення і всі дії при циклі
      this.current = this.from;
      return this;
   },
   next() {
      //Функція що буде повторюватись при кожній ітерації
...
let myObject = {
   ...
   next() {
      //Функція що буде повторюватись при кожній ітерації
      return this.current < this.category.length
         ? { value: this.category[this.current++].name, done: false }
         : { done: true };
   },
};
